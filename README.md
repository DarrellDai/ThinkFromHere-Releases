...
桌面版 **0.2.1** · Android **0.2.1 (201)** · iOS **0.2.1 (201) 模拟器测试包**

ThinkFromHere 0.2.1 改进多账号登录与切换体验，桌面、Android 和 iOS 模拟器版本统一为 0.2.1。

## 主要更新

- 登录、添加账号和切换账号时保持桌面 App 窗口打开，在原窗口内加载目标账号工作区。
- 修复添加账号后短暂闪回初始“恢复登录”界面的问题。
- 手机切换账号时取消旧账号的同步网络请求，本地账号识别不再等待网络刷新令牌；未同步数据保留在原账号。
- 登录后才显示画布；退出当前账号保留其他已保存账号，非当前账号可通过右键或长按菜单移除。
- 改进添加账号网页登录布局，使用 App 图标；返回 App 后恢复登录按钮，避免长期停留在 Signing in。
- 完善账号隔离、切换失败回退和旧请求清理。

## 下载与升级

- Windows x64、macOS universal、Linux x64、Android：下载对应安装包及 `.sha256` 校验文件。
- Android 使用正式包名和固定 Release 签名，versionCode 201，可覆盖升级同签名正式版；测试版仍独立安装。
