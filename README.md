...
桌面版 **0.2.0** · Android **0.2.0 (200)** · iOS **0.2.0 (200) 模拟器测试包**

ThinkFromHere 0.2.0 是一次跨设备 Codex 工作流更新，桌面、Android 和 iOS 模拟器版本统一为 0.2.0。

## 主要更新

- 手机在原有画布和卡片详情中远程操作电脑上的 Codex；同一 ThinkFromHere 账号、正在运行应用的电脑会自动出现。
- 手机可选择在线电脑，新建 Codex Session；可调模型、权限与思考程度，发送、追加、排队和停止，并回复模型的补充问题。账号授权和权限审批仍在电脑完成。
- 新建远程会话只同步目标画布，减少全量历史等待；改进手机回复定位和紧凑布局。
- 手机可安全切换 ThinkFromHere 账号；每个账号使用独立本机数据，旧账号的记录和 Key 保留。
- 手机与电脑分别提供 Chat / Codex 通知同步开关，新安装默认开启；保留已保存的关闭选项。
- Codex 欢迎界面直接选择 CLI 登录或独立账号登录；修正 Ubuntu 登录完成页可能唤起未安装应用的问题。
- 修复空设置值导致的云同步失败，完善通知、已读和账号隔离的回归检查。

## 下载与升级

- Windows x64、macOS universal、Linux x64、Android：下载对应安装包及 `.sha256` 校验文件。
- Android 0.2.0 使用固定 Release 签名，versionCode 200；同包名、同签名的现有版本可以覆盖升级。
- iOS ZIP 仅用于 macOS 的 iOS Simulator，不能安装到 iPhone 真机。
- 手机远控需要电脑升级到 0.2.0、运行 ThinkFromHere 并登录同一云账号；Codex 账号与云账号相互独立。
- 应用使用 `com.darrelldai.thinkfromhere` 标识。仍使用旧 forkchat 包名或目录的安装不会自动迁移本机数据，请保留旧数据。
