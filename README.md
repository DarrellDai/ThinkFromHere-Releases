...
桌面版 **0.2.2** · Android **0.2.2 (202)** · iOS **0.2.2 (202) 模拟器测试包**

ThinkFromHere 0.2.2 支持 Codex 后台执行与跨账号通知跳转，优化手机账号切换并修复登录恢复，桌面、Android 和 iOS 模拟器版本统一为 0.2.2。

## 主要更新

- Codex 任务在正常关闭窗口、进入后台或切换账号后继续执行，返回后查看最新进展。
- 手机切换账号复用已有工作区，减少重载与重复初始化。
- 已保存账号先恢复本地工作区，云端令牌刷新不再阻塞进入画布；凭据恢复失败时提供重试和重新登录。
- 点击系统完成通知，自动切换到通知所属的已登录账号并打开对应卡片。
- 后台任务按账号隔离凭据、数据库和附件密钥；执行任务的电脑需保持运行。

## 下载与升级

- Windows x64、macOS universal、Linux x64、Android：下载对应安装包及 `.sha256` 校验文件。
- Android 使用正式包名和固定 Release 签名，versionCode 202，可覆盖升级同签名正式版；测试版仍独立安装。
