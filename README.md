...
桌面版 **0.2.3** · Android **0.2.3 (203)** · iOS **0.2.3 (203) 模拟器测试包**

ThinkFromHere 0.2.3 改善画布启动恢复、登录与通知。

## 主要更新

- 修复画布初始化与同步刷新竞争；启动本地列表为空时，在首次同步后自动打开已有画布。
- 加载失败提供重试入口，页面渲染异常提供重新打开画布入口。
- Inbox 新增一键清空图标，清空完成通知时同步标为已读并取消紫色高亮。
- 抑制导入历史、已读卡片和过期任务的完成推送，避免手机集中收到旧通知。
- 改善浏览器登录链接处理与通知音频超时判定。

## 下载与升级

- Windows x64、macOS universal、Linux x64、Android：下载对应安装包及 `.sha256` 校验文件。
- Android 使用正式包名和固定 Release 签名，versionCode 203，可覆盖升级同签名正式版；测试版仍独立安装。
